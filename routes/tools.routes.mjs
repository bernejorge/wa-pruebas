...
router.get('/getGroupByCentro', async (req, res) => {
   try {
     ...
     const data = await getGroupByCentro(inputText, 'profesionales_servicio_centro_hp0637');
     res.json(data);
   } catch (error) {
 ...
 router.get('/getGroupByCentrohrf', async (req, res) => {
   try {
     ...
     const data = await getGroupByCentro(inputText, 'profesionales_servicio_centro_hrf1325');
     res.json(data);
   } catch (error) {
     console.error('Error en la ruta /getGroupByCentro:', error);
     res.status(500).json({ error: 'Error interno del servidor.' });
